...
# ===================== 常量与参数 =====================
DEFAULT_MTU = 1500               # UDP 整包上限（含 IP/UDP/RTP/负载）
...
class NetGearUDP:
    ...
    def send(self, frame: Union[bytes, bytearray, memoryview],
        ...
        # ======================= 新增：整帧直发路径（实例级开关） =======================
        # 使用实例变量 self._deliver_per_packet 控制直发；若未定义，则默认 False。
        if getattr(self, "_deliver_per_packet", False):
            max_data_udp_only = self._mtu - 28  # 仅UDP头的负载上限
            if len(data) > max_data_udp_only:
                # 保持原风格：给出明确错误信息并截断到单包上限（如需严格失败，可改为 raise）
                print(
                    f"[ERROR]: deliver_per_packet=True 但负载过大：len(data)={len(data)} "
                    f"> 单包上限 {max_data_udp_only}（= mtu - 28(IP+UDP)）。"
                    "请增大 mtu 或将 _deliver_per_packet 设为 False。"
                )
                data = data[:max_data_udp_only]
            ...
            self._sent_octets += len(data)  # SR统计按RTP负载字节数，不含RTP头
