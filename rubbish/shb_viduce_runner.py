...
def save_rgb_np(x: np.ndarray, path: Path):
    path.parent.mkdir(parents=True, exist_ok=True)
    Image.fromarray(ensure_uint8(x)).save(path)
...
class GraceBundle:
    ...
    def _call_decode(self, eframe, ref_img: Image.Image) -> np.ndarray:
        ...
        decoded = self.ae.decode_frame_from_compressed(eframe)
    ...
    def encode_keyframe(self, ref_np: np.ndarray, cur_np: np.ndarray, is_iframe: bool):
        ...
        size, eframe_original, compressed_data_original = self._call_encode(is_iframe, ref_img, cur_img)
        # print(eframe_original.shapex, eframe_original.shapey)
        
        # 保存编码结果到临时文件
        out_bin_path = Path('/home/wxk/workspace/nsdi/Viduce/out/temp_encode.bin')
        # === 修正点：写入前先清空旧文件，避免读到历史记录 ===
        try:
            out_bin_path.unlink()
        except FileNotFoundError:
            pass
        append_grace_record(out_bin_path, eframe=eframe_original, compressed_data=compressed_data_original)

        
        # 读取保存的结果
        loaded_data = None
        for record_type, record_data in iter_read_grace_records(out_bin_path):
            if record_type in ['I', 'P']:
                loaded_data = record_data
                break
        
        if loaded_data is None:
            raise RuntimeError("无法从保存的文件中读取编码数据")
        
        # 删除原始eframe变量（保留原始对象用于解码）
        del eframe_original
    ...
    def decode_mv_to_frame(self, eframe, refer_frame): 
        """
        把解出的 MV 帧统一转为 numpy (H, W, C) float32 in [0,1]
        """
        mv = self.ae.decode_frame_from_mv(eframe)               # 解码得到 motion vector
        img = self.ae.decode_motion_vector(mv, refer_frame)     # 解码得到 torch.Size([1, 3, H, W])
        import torch
        # 确保张量在 CPU，并去掉 batch 维度 (1,3,H,W) -> (3,H,W)
        t = img.detach().cpu().squeeze(0)  

        # 转换为 (H,W,C)
        arr = t.permute(1, 2, 0).float().numpy()  
...
def _write_record(fh, ftype: int, body_bytes: bytes):
    """内部工具：按统一 header 写入一条记录。"""
    header = struct.pack('<4sBBHI', MAGIC, VERSION, ftype, 0, len(body_bytes))
    fh.write(header)
    fh.write(body_bytes)
...
def iter_read_grace_records(bin_path: Path):
    """
    迭代读取记录（兼容 v3 / v4）：
      - ('mv', EncodedFrameLike) —— eframe.frame_type='MV'，eframe.shapey=4-tuple，eframe.mv_stream=bytes
      - ('I',  EncodedFrameLike) —— shapex/shapey 为 int；不再提供 frame_id
      - ('P',  EncodedFrameLike) —— P 级 shapex/shapey 为 4-tuple；ipart 的 shapex/shapey 为 int；shapez 为 4-tuple
                                   若文件为 v4，还会把 eframe.mxrange_res 设为 float；v3 则为 None
    """
    with bin_path.open('rb') as fh:
        hdr_sz = struct.calcsize('<4sBBHI')
        while True:
            hdr = fh.read(hdr_sz)
            if not hdr:
                break
            magic, ver, ftype, _reserved, payload_len = struct.unpack('<4sBBHI', hdr)
            if magic != MAGIC:
                raise RuntimeError(f'文件头不匹配或版本不支持：magic={magic} ver={ver}')
            body = fh.read(payload_len)

            if ftype == FTYPE_MV:
                syN, syC, syH, syW = struct.unpack_from('<4i', body, 0)
                mv_len, = struct.unpack_from('<I', body, 16)
                mv_bytes  = body[20:20+mv_len]
                eframe = EncodedFrameLike(
                    frame_type='mv',
                    shapex=(syN, syC, syH, syW),
                    mv_stream=bytes(mv_bytes)
                )
                # MV 记录无 mxrange_res
                setattr(eframe, 'mxrange_res', None)
                yield ('mv', eframe)
                continue

            if ftype == FTYPE_I:
                off = 0
                sx, sy = struct.unpack_from('<ii', body, off); off += 8
                code_len, = struct.unpack_from('<I', body, off); off += 4
                ...
                setattr(eframe, 'mxrange_res', None)  # I 帧没有该字段
                yield ('I', eframe)
                continue

            if ftype == FTYPE_P:
                off = 0
                # P 级 shapex/shapey（四元组）+ z 的四元组
                psxN, psxC, psxH, psxW = struct.unpack_from('<4i', body, off); off += 16
                psyN, psyC, psyH, psyW = struct.unpack_from('<4i', body, off); off += 16
                zN, zC, zH, zW         = struct.unpack_from('<4i', body, off); off += 16

                # ipart 的 shapex/shapey（int）+ 偏移宽高（int）
                ip_sx, ip_sy           = struct.unpack_from('<ii', body, off); off += 8
                off_w, off_h           = struct.unpack_from('<ii', body, off); off += 8

                # 读取 mxrange_res(f32)
                mxrange_res, = struct.unpack_from('<f', body, off); off += 4

                # 四段 offset + 四段 length
                res_off, mv_off, z_off, ip_off = struct.unpack_from('<IIII', body, off); off += 16
                res_len, mv_len, z_len, ip_len = struct.unpack_from('<IIII', body, off); off += 16

                data_base = off
                blob = memoryview(body)[data_base:]
                res = bytes(blob[res_off:res_off+res_len])
                mv  = bytes(blob[mv_off:mv_off+mv_len])
                z   = bytes(blob[z_off:z_off+z_len])
                ip_code = bytes(blob[ip_off:ip_off+ip_len])

                ipart = IPartLike(
                    code=ip_code,
                    shapex=ip_sx, shapey=ip_sy,
                    offset_width=off_w, offset_height=off_h
                )
                eframe = EncodedFrameLike(
                    frame_type='P',
                    shapex=(psxN, psxC, psxH, psxW),
                    shapey=(psyN, psyC, psyH, psyW),
                    ipart=ipart,
                    res_stream=res, mv_stream=mv, z_stream=z,
                    shapez=(zN, zC, zH, zW), mxrange=mxrange_res
                )
                yield ('P', eframe)
                continue

            raise RuntimeError(f'未知 ftype: {ftype}')
...
def run_g5_pix2pix_grace(args, frames: List[Path], pix: Pix2PixRunner, grace: GraceBundle) -> List[FrameRec]:
    ...
    for gi in range(0, (len(frames)+4)//5):
        ...
        # # 保存 pix2pix 结果（IO 不纳入计时）
        save_rgb_np(dec0_np, args.out_dir / f'pix2pix_{f0.name}')
        # 保存编码后的关键帧
        ...
        for j, fj in enumerate(group_files[1:], start=1):
            ...
            # 从out_bin_path读取mv_j并将它恢复为原来的shape
            loaded_data = None
            for record_type, record_data in iter_read_grace_records(out_bin_path):
                if record_type == 'mv':
                    loaded_data = record_data
            ...
            prev_p2p_np = decoded_frame
            save_rgb_np(decoded_frame, args.out_dir / f'pix2pix_frame_{s+j}.png')
            # print(prev_p2p_np.shape, decoded_frame.shape)
            # from torchvision.utils import save_image
            # full_out = torch.clamp(decoded_frame, 0, 1)
            # save_image(full_out, args.out_dir / f'pix2pix_frame_{s+j}.png')
