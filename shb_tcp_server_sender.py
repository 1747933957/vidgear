...
def send_file_via_netgear(file_path: Path, frame_id: int):
    """
    修改点：
    - 第一次调用时建立全局 NetGear（TCP/REQ）
    - 每次发送都附带 message={"frame_id": frame_id}
    - 发送的帧用 np.uint8 的一维数组承载原始字节
    """
    global pre_time  # 声明使用全局变量
    try:
        # 读取文件
        with open(file_path, "rb") as f:
            file_data = f.read()
        # 将 bytes 打包为一维 uint8 数组，保持"无压缩路径"
        frame_array = np.frombuffer(file_data, dtype=np.uint8)
        net = _ensure_global_tcp_net()
        cur_time = time.time()
        if pre_time is not None and cur_time - pre_time < 0.03:
            time.sleep(max(0.0, 0.03 - (cur_time - pre_time)))
        net.send(frame_array, message={"frame_id": int(frame_id)})
        pre_time = time.time()
...
'''
python shb_tcp_server_sender.py \
